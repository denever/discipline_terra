...
from django.forms import ClearableFileInput, EmailInput, NumberInput, Select, TextInput
from django.utils.translation import ugettext as _
from localflavor.it.forms import ITVatNumberField

from invoices.models import Invoice, InvoiceHeading, Item, Order, Payment
    ...
    def clean(self):
        ...
        if pieces > tot_qty:
            raise forms.ValidationError(
                f"Value {pieces}s exceed quantity available"
                "for this product: {tot_qty}s"
            )
        return cleaned_data
...
class HeadingForm(forms.ModelForm):
    tax_code = ITVatNumberField(
        label=_("Tax code or Vat code"),
